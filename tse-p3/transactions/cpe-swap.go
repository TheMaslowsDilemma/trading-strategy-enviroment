...
func (tx CpeSwap) Apply(tick uint64, l ledger.Ledger) (*ledger.Ledger, error) {
	...
	pyr_wlt = ledger_delta.GetWallet(pyr_wlt_addr)
	...
	if tx.SymbolIn == exg.ReserveA.Symbol {
		...
		return nil, fmt.Errorf("slippage: amt_out %v < min %v", amt_out, tx.AmountMinOut)
		}
